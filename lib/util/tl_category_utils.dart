...
class TLCategoryUtils {
  ...
  static List<Map<String, dynamic>> categoryArrayToJson(
      {required List<TLToDoCategory> categoryArray}) {
    return categoryArray.map((cat) => cat.toJson()).toList();
  }

  // JSON をカテゴリー配列に変換
  static List<TLToDoCategory> jsonToCategoryArray(
      {required List<dynamic> jsonCategoryArrayData}) {
    return jsonCategoryArrayData
        .map((item) => TLToDoCategory.fromJson(item as Map<String, dynamic>))
        .toList();
  }
  ...
  // JSON を Map に変換
  static Map<String, List<TLToDoCategory>> jsonToSmallCategories({
    required Map<String, dynamic> jsonSmallCategoriesData,
  }) {
    final Map<String, List<TLToDoCategory>> result = {};
    ...
    jsonSmallCategoriesData.forEach((bigCategoryId, jsonCategories) {
      if (jsonCategories is List<dynamic>) {
        result[bigCategoryId] = jsonCategories
            .map((jsonCategory) =>
                TLToDoCategory.fromJson(jsonCategory as Map<String, dynamic>))
            .toList();
      } else {
